...
describe('Plugin mongooseI18nExtra', () => {
  beforeAll(async () => {
    await mongoose.connect('mongodb://localhost:27017/mongoose-i18n', { useNewUrlParser: true });
    ...
    schema.plugin(mongooseI18nExtra, { languages: ['en', 'de', 'fr'], defaultLanguage: 'en' });
  ...
  describe('Create / update', () => {
    ...
    test('should be able to create a document with i18n fields', async () => {
      let sample = new Sample({ _id: sampleId, name: 'test', description: 'desc en' });
      ...
      expect(sample._i18n).toEqual({"de": {description: null }, "en": {description: "desc en" }, "fr": {description: null }});
    });
    ...
    test('should be able to update a document with i18n fields', async () => {
      ...
      expect(sample._i18n).toEqual({"de": {description: null }, "en": {description: "desc en" }, "fr": {description: "desc fr"}});
  ...
  test('should set default value if not default value is empty', async () => {
    ...
    expect(sample._i18n).toEqual({"de": {description: null }, "en": {description: "test fr"}, "fr": {description: "test fr"}});
  });
  ...
  test('should support multiple translations', async () => {
    ...
    expect(sample.toJSON()).toEqual({...valueStoredInDatabase, _i18n: i18nValuesWithGetter});
    expect(sample.toObject({ getters: true})).toEqual({...valueStoredInDatabase, id: sample._id.toString(), _i18n: i18nValuesWithGetter, ...virtualFields});
    expect(sample.toObject({ virtuals: true})).toEqual({...valueStoredInDatabase, id: sample._id.toString(), _i18n: i18nValuesWithGetter, ...virtualFields});

    expect(sample._i18n).toEqual({"de": {description: "test de"}, "en": {"description": "test en"}, "fr": {description: "test fr"}});
    ...
    expect(sample._i18n).toEqual({"de": {description: "test de2"}, "en": {"description": "test en2"}, "fr": {description: "test fr2"}});
  });
